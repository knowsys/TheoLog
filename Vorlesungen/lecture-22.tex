...
Offenbar ist Schließen in Logik zweiter Stufe mindestens genauso schwer wie in Logik zweiter Stufe. Tatsächlich ist es noch deutlich schwerer:\medskip
...
(\text{Erreichbar}(\Scodelit{\texttt{helmholtzstr}}) \wedge{}\\
 &(\text{Erreichbar}(x)\wedge \text{verbindung}(x,y,z)\to \text{Erreichbar}(y)) )\to \text{Erreichbar}(x)
\Big)
\end{align*}
Die Formel hat eine freie Variable $x$ und stellt $\text{Erreichbar}$ als Prädikaten-Variable dar.
Ein Fakt $\text{Erreichbar}(a)$ folgt aus dem ursprünglichen Programm über einer Datenbank $\Inter$ genau dann wenn $\Inter$ die Formel mit Variablenbelegung $x\mapsto a$ erfüllt.
}
