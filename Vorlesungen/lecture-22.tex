...
\item Für jede Produktionsregel $\Snterm{A}\to v$ mit $v=v_1\ldots v_n\in(\Sigma\cup V)^*$ sei $A(x_0,x_n)\leftarrow v_1(x_0,x_1)\wedge\ldots\wedge v_n(x_{n-1},x_n)$ eine Datalog-Regel
...
\emph{Syntax:} Wie in Prädikatenlogik, aber mit quantifizierten Prädikatenvariablen.\\
\textcolor{devilscss}{(Die Stelligkeit einer Prädikatenvariablen muss jeweils klar festgelegt werden)}\medskip
...
\Big)
\end{align*}
Die Formel hat eine freie Variable $v$ und stellt $\text{Erreichbar}$ als Prädikatenvariable dar.
