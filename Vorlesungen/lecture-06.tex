...
\defineTitle{6}{Einführung in die Komplexitätstheorie}{4. Mai 2026}
...
\begin{frame}\frametitle{Wiederholung: Definition TM}
...
\defbox{Eine \redalert{nichtdeterministische Turingmaschine} (NTM) modifiziert die DTM wie folgt:
\begin{itemize}
...
% \alert{Dabei bedeutet $\delta(q,a)=\tuple{p,b,D}$ (bzw. $\tuple{p,b,D}\in\delta(q,a)$:}\\
% \hspace{5mm}"`Liest die TM in Zustand $q$ unter dem Lese-/Schreibkopf ein $a$,\\
% \hspace{5.8mm}dann wechselt sie (nichtdeterministisch) zu Zustand $p$, überschreibt das $a$ mit $b$\\
...
\examplebox{\emph{Beispiel:} Ein Graph kann als Adjazenzmatrix kodiert werden ($O(n^2)$ Speicher) oder z.B. auch als Adjazenliste ($O(e\cdot \log v)$ Speicher für $e$ Kanten und $v$ Knoten). Letzteres ist deutlich effizienter für lichte Graphen, aber der Unterschied bleibt stets polynomiell.}\bigskip
...
% \only<-2>{~\\~\\~\\}%
\only<3->{\\Seit Jahrzehnten suchen Forschende nach besseren
Lösungen:\\
...
$\Scomplclass{DTIME}(n^{\redalert{2,374}})$ [Stothers, 2010],
$\Scomplclass{DTIME}(n^{\redalert{2,3729}})$ [Williams, 2011],
$\Scomplclass{DTIME}(n^{\redalert{2,3728639}})$ [Le Gall, 2014],
$\Scomplclass{DTIME}(n^{\redalert{2,3728596}})$ [Alman \& Williams, 2020],
$\Scomplclass{DTIME}(n^{\redalert{2,371866}})$ [Duan et al., 2022],
$\Scomplclass{DTIME}(n^{\redalert{2,371552}})$ [Williams et al., 2024]
und
$\Scomplclass{DTIME}(n^{\redalert{2,371339}})$ [Alman et al., 2024].
}
% \only<-3>{\phantom{Vermutete} \phantom{optimale} \phantom{Lösung:} \phantom{$\Scomplclass{DTIME}(n^{\redalert{2}})$.}}%
\only<4->{\\\emph{Vermutete optimale Lösung:} $\Scomplclass{DTIME}(n^{\redalert{2}})$.}
}
...
Vorlesung 4 kennengelernt: Erfüllbarkeit von propositionaler Horn-Logik. Unser Resolutionsalgorithmus
