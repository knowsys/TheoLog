...
\defineTitle{15}{Logisches Schließen}{15. Juni 2026}
...
% \only<1->{\includegraphics[height=5cm]{../../internal/wahlergebnisse-BT-2021-Dresden-II.png}}%
% \end{minipage}%\pause%
% \begin{minipage}{4cm}
% \vspace{4cm}
% Erstimmen,\\
...
\examplebox{\emph{Beispiel:} "`Es gibt nur einen Rudi Völler"' (erstes Suchergebnis von Google bei Eingabe "`Es gibt nur einen"', Stand Juni 2026)
\begin{align*}
...
$\Leftarrow$ folgt, weil $\Jnter\models\mathcal{EQ}_{\Slang{R}}$, sodass $\eqpred^\Jnter$ eine Kongruenzrelation ist
\bigskip\pause
...
\begin{itemize}
\item \alert{Klassische Induktion:} Ist $E$ eine Eigenschaft, sodass gilt:
...
\item \alert{Strukturelle Induktion auf Formeln:} Ist $E$ eine Eigenschaft, sodass gilt:
...
\examplebox{\emph{Induktion auf der Insel der Wahrheitssager und Lügner:} Ein Einwohner verkündet: "`Was ich jetzt sage, das habe ich schon einmal gesagt."' Welchen Typ hat er?}

\end{frame}

\begin{frame}\frametitle{Modelltheorie versus Axiomatisierung}

\alert{Der Fall von Gleichheit illustriert zwei praktische Methoden, um Prädikatenlogik für besondere Anwendungen anzupassen.}
\bigskip

\emph{Einschränkung der Modelltheorie:}
\begin{itemize}
\item Erlaube nur Modelle, die bestimmte zusätzliche Eigenschaften haben
\item Beispiel: $\approx$ muss immer als Identität auf $\Delta^\Inter$ interpretiert werden
\item Weniger Modelle $\leadsto$ mehr logische Schlussfolgerungen
\item Vorteile: sehr ausdruckstark, unter Umständen einfacher
\item Nachteile: spezielle Algorithmen erforderlich, unter Umständen noch viel schwerer
\end{itemize}\bigskip

\emph{Erweiterung der logischen Theorie:}
\begin{itemize}
\item Ergänze die betrachtete Formelmenge durch zusätzliche Sätze
\item Beispiel: axiomatisiere $\eqpred$ durch die Hilfstheorie $\mathcal{EQ}_{\Slang{R}}$
\item Mehr Sätze $\leadsto$ mehr logische Schlussfolgerungen
\item Vorteile: Standardalgorithmen anwendbar, leicht anpassbar (durch Modellierende)
\item Nachteile: nicht jede Modelltheorie ist axiomatisierbar, Standardalgorithmen nicht optimiert für Spezialfälle
\end{itemize}
