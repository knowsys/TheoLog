...
\defineTitle{19}{Resolution}{28. Juni 2017}
...
\frame{\begin{center}
\large 
We have just learned from Le Bérarde that a new accident has occurred on the Pelvoux plateau: a young man, part of a three-man company from Lyons, has fallen to his death.\bigskip
...
\item Fall 1: $\Inter,\Zuweisung\models A_1\sigma$ ($=A_2\sigma=\ldots=A'_m\sigma)$. Dann gilt $\Inter,\Zuweisung\models L'_1\sigma\vee\ldots\vee L'_\ell\sigma$, und damit $\Inter,\Zuweisung\models K$\pause
...
A rescue party has left to seek the body, which it hopes to reach today. \bigskip
...
(Portrait aus Vorlesung wegen rechtlicher Bedenken nicht inkludiert)
...
\examplebox{Beispiel: Für die Formel $F=p(f(x),y,g(z))$ ergibt sich
...
Bei Herbrandinterpretationen kann man semantische Elemente (wie sie in Zuweisungen vorkommen) durch syntaktishe Elemente (wie sie in Substitutionen vorkommen) ausdrücken:\medskip
...
{\footnotesize(Direkt aus Definition für Atome $G$; die Aussage kann leicht auf größere Boolsche Verknüpfungen von Atomen verallgemeinert werden -- formal durch strukturelle Induktion)}\pause
...
% \begin{frame}\frametitle{Löwenheim}
% 
% 
% 
% \end{frame}
...
Jede erfüllbare prädiktenlogische Formel hat ein abzählbares Modell (d.h. eines mit abzählbarer Domäne).}
...
\item Der \alert{nach oben gerichtete Satz von Löwenheim und Skolem}  (Upward Löwenheim-Skolem Theorem) besagt, dass jede erfüllbare Formel auch in Modellen beliebig großer Kardinalität erfüllbar ist
...
% 
% Folie \ref{frame_herbrand}: Fotografie, 1931, (c) Natasha Artin Brunswick. Alle Rechte vorbehalten
% 
% \end{frame}
