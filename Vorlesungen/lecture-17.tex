...
\defineTitle{17}{Funktionen und Normalformen}{22. Juni 2026}
...
In Vorlesung 13 betrachteten wir den Fall, dass alle Einwohner einer Insel sagen,\\
"`Wir sind hier alle vom gleichen Typ."'\bigskip
...
\theobox{\emph{Satz:} Sei $F$ eine bereinigte Formel in NNF. Eine zu
...
\begin{itemize}
\item Für eine Formel $F$ sei $J(F)$ die Zahl der Junktoren in $F$
...
\examplebox{\emph{Beispiel:} Den Satz $\forall x.\exists y.\textsf{hatVater}(x,y)$ ("`Jeder hat einen Vater"') könnte formuliert werden als $\forall x.\textsf{hatVater}(x,\textsf{vater}(x))$. Letzteres ist so zu verstehen:
\begin{itemize}
...
eine der folgenden rekursiven Bedingungen erfüllt:
\begin{itemize}
...
ist ein Paar $\tuple{\Delta^\Inter,\cdot^\Inter}$ bestehend aus einer nichtleeren Grundmenge $\Delta^\Inter$ (der \redalert{Domäne}) und einer \redalert{Interpretationsfunktion} $\cdot^\Inter$, welche:
\begin{itemize}
...
{\tiny
\emph{Achtung:} Es gibt oft mehr mögliche Modelle als man zunächst denkt. Prädikatenlogik eignet sich zur Beschreibung algebraischer Strukturen allgemein, aber nicht zur Beschreibung ganz
...
\examplebox{\emph{Beispiel:} Für die Formel $\forall x.\exists y.\forall z.\exists v.p(x,y,z,v)$ ergibt sich:
\begin{tabular}{rl}
...
\examplebox{\emph{Beispiel:} Die Formel $\forall x.\exists y.(p(x)\to p(y))$ ist eine Tautologie,
...
\theobox{\emph{Satz:} Eine Formel in Pränexform $F$ ist genau dann erfüllbar, wenn auch die Skolemisierung von $F$ erfüllbar ist.}\medskip
...
\theobox{\emph{Satz:} Für jede Formel der Prädikatenlogik mit Funktionen gibt es eine erfüllbarkeitsäquivalente Formel ohne Funktionssymbole, welche in linearer Zeit berechnet werden kann.}
...
\theobox{\emph{Satz:} Die so aus einer Formel $F$ gebildete KNF ist erfüllbar genau dann, wenn $F$ erfüllbar ist.}
