...
\defineTitle{12}{\PSpace-Vollständigkeit}{4. Juni 2026}
...
\item Rekursionen in Zeilen \texttt{05} und \texttt{07} können der Reihe nach abgearbeitet werden, wobei Speicher wiederverwendet wird
...
\item Zeit ist problematisch: in \PSpace kann eine TM exponentiell lang laufen
...
\alert{04}~~~~~~result = result \Scode{OR} \textsc{aCannotWin}($G$,$m$,Visited${}\cup\{m\}$)\\
...
\alert{09}~~~~~~result = result \Scode{AND} \textsc{eCanWin}($G$,$m$,Visited${}\cup\{m\}$)\\
...
"`Gegeben eine det.\ Turingmaschine $\mathcal{M}$, eine Eingabe $w$, und ein `Füllwort' $\Sterm{a}^m$ der Länge $m$, wird $\mathcal{M}$ das Wort $w$ in Zeit $\underbrace{2^{.^{.^{.^{2^{m}}}}}}_{k\text{-mal 2}}$ akzeptieren?"'
{\tiny (Warum ist das $k$-$\complclass{Exp}$-vollständig? Zeigen Sie Inklusion und Schwere.)}
\bigskip\pause
...
\begin{itemize}
\item Erreichbarkeit in ungerichteten Graphen (Omer Reingold, 2005)
