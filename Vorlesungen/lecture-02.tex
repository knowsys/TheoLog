...
\defbox{Eine TM $\Smach{M}$ \redalert{berechnet} eine partielle Funktion $f_{\Smach{M}}:\Sigma^*\to\Sigma^*$ wie folgt.
...
Wir interessieren uns für Funktionen und Sprachen, die man berechnen kann:

\defbox{Eine totale oder partielle Funktion $f$ heißt \redalert{berechenbar}, wenn es eine TM $\Smach{M}$ gibt, die $f$ berechnet, d.h. mit $f=f_{\Smach{M}}$.}
...
Frage: Welche Menge zählt diese TM auf?\pause\bigskip

Antwort: $\{ \Sterm{a}^{2n+1}\Sterm{b}\mid 0\leq n\}$
\bigskip
...
\item Für jede natürliche Zahl $n\geq 1$:
	...
	\item Für jedes $i\in\{1,\ldots, n\}$:
		\begin{itemize}
		\item Simuliere $\Smach{M}$ auf Eingabe $w_i$ für $i$ Schritte
...
$\textsf{Graph}_f=\{\tuple{w,f(w)}\mid w\in\Sigma^*\}$ semi-entscheidbar ist.
...
Das Argument zeigt zudem auch, dass die meisten Sprachen nicht einmal semi-entscheidbar (Kontrollfrage: Warum?).
...
\begin{itemize}
\item Man kann die Relation $\in$ zwischen Wörtern und Sprachen jetzt als unendliche Tabelle darstellen:
%
...
\begin{frame}\frametitle{Nichtwissen $\neq$ Unentscheidbarkeit}
...
\examplebox{Beispiel: Sei $\Slang{L}_\pi$ die Menge aller endlichen Ziffernfolgen, die in der Dezimaldarstellung von $\pi$ vorkommen.\\[1ex]
...
\examplebox{Beispiel: Sei $\Slang{L}_{\pi7}$ die Menge aller Ziffernfolgen der Form $7^n$, die in der Dezimaldarstellung von $\pi$ vorkommen.\\[1ex]

\only<3->{
$\Slang{L}_{\pi7}$ ist entscheidbar:
\begin{itemize}
\item Möglichkeit 1: $\pi$ enthält beliebig lange Ketten der Ziffer $7$. Dann wird $\Slang{L}_{\pi7}$ durch eine TM entschieden, die alle Wörter der Form $7^n$ akzeptiert.
\item Möglichkeit 2: $\pi$ enthält Ketten der Ziffer $7$ nur bis zu einer maximalen Länge $\ell$. Dann wird $\Slang{L}_{\pi7}$ durch eine TM entschieden, die alle Wörter der Form $7^n$ mit $n\leq\ell$ akzeptiert.
\end{itemize}
...
\begin{frame}\frametitle{Ein erstes unentscheidbares Problem}
...
\begin{frame}\frametitle{Ein erstes unentscheidbares Problem (2)}

\emph{Frage:} Falls eine TM \alert{mit $n$ Zuständen} und \alert{einem zweielementigem Arbeitsalphabet $\Gamma=\{\Sterm{x}, \blank\}$} auf einem \alert{leeren Band} anhält, wie lange kann das im schlimmsten Fall dauern?\\
\bigskip\pause
...
Anmerkung: Der genaue Wert von $\bbfunc(n)$ hängt von Details der TM-Definition ab.\\
...
\item Sei $\Smach{M}_{\times2}$ eine TM, welche die Funktion $\Sterm{x}^i\mapsto\Sterm{x}^{2n}$ berechnet.\pause
