...
\defbox{Die Menge der \redalert{prädikatenlogische Formeln} ist induktiv definiert:
\begin{itemize}
...
bezeichnen, was zu unterschiedlichen Teilformenl führt.}
...
\examplebox{Beispiel: Eine Variable kann gleichzietig frei und gebunden in einer Formel 
...
einer geschlossenen Formeln anschaut (dies tun wir in  vielen Defintionen und
Beweisen).\bigskip
...
\item um Atome $\alpha$ auf Wahrheitswerte $\alpha^{\Inter,\Zuweisung}\in\{\myfalse,\mytrue\}$ abzubilden
\end{enumerate}}
...
Smullyan\footnote{Raymond Smullyan: A Beginner's Guide to Mathematical Logic, Dover 2014} fragte die Bewohner nach ihren Rauchgewohnheiten.\pause
...
%
\item Auf \redalert{Insel C} antwortete jeder der Bewohner:\\ \alert{"`Falls ich rauche, dann raucht jeder hier."'}
%
\item Auf \redalert{Insel D} antwortete jeder der Bewohner:\\ \alert{"`Einige hier rauchen, aber ich nicht."'}
\end{itemize}
