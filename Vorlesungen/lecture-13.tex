...
\item man damit keine zusätzliche Ausdrucksstärke erhält und
...
\item Die freien Vorkommen in $\forall x.F$ und $\exists x.F$ sind dieselben 
wie in $F$, ohne die Vorkommen von $x$ in $F$
\end{itemize}
...
\begin{itemize}
\item Was bedeutet $p$?\\
	\visible<2->{Prädikatensymbole stehen für Relationen gleicher Stelligkeit}
\item Was bedeutet $a$?\\
