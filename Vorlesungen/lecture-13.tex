...
\defineTitle{13}{Prädikatenlogik: Syntax und Semantik}{8. Juni 2026}
...
\begin{frame}\frametitle{Überblick Aussagenlogik}

\begin{center}
\alert{Vorlesung 1}\\
Warum Logik? $\bullet$ Aussagenlogik Syntax \& Semantik $\bullet$ Anna lügt\\[1ex]
\alert{Vorlesung 2}\\
Aussagenlogische Konsequenzen \& Äquivalenzen $\bullet$ Modellierung (Sudoku) $\bullet$ Z3 \\[1ex]
\alert{Vorlesung 3}\\Logisches Schließen $\bullet$ NNF, DNF \& KNF\\ Strukturelle (Tseitin-)Transformation bei KNF \\[1ex]
\alert{Vorlesung 4}\\Aussagenlogische Resolution $\bullet$ Korrektheit der Resolution $\bullet$ Horn-Logik \\[1ex]
\alert{Vorlesung 5}\\Splitting, Backtracking und DPLL $\bullet$ SMT-Solving\\ Schließen als Wortproblem
\end{center}
...
\begin{center}
\alert{Vorlesung 6}\\
Königsberger Brücken $\bullet$ Beschränkte TMs $\bullet$ Robuste Komplexität\ghost{en}\\[0.8ex]
\alert{Vorlesung 7}\\
Zeit vs. Raum $\bullet$ \complclass{P} und \complclass{L}  $\bullet$ Polynomielle Reduktionen \\[0.8ex]
\alert{Vorlesung 8}\\
Komplexität von $\Slang{SAT}$ $\bullet$ \complclass{NP} $\bullet$ Nachweispolynomialität \\[0.8ex]
\alert{Vorlesung 9}\\
Coles Beweis ohne Worte $\bullet$ \complclass{NP}-Vollständigkeit $\bullet$ Cook-Levin $\bullet$ $\Slang{SAT}\leq_p \Slang{Clique}\leq_p \Slang{Unabhängige Menge}$ \\[0.8ex]
\alert{Vorlesung 10}\\
$\Slang{SAT}\leq_p \Slang{Teilmengen-Summe}\leq_p \Slang{Rucksack}$ $\bullet$ Pseudopolynomialität $\bullet$ Fake News zu \complclass{NP} \\[0.8ex]
\alert{Vorlesung 11}\\
\complclass{NL} und Erreichbarkeit $\bullet$ Schach ist schwer $\bullet$ \PSpace und QBF \\[0.8ex]
\alert{Vorlesung 12}\\
...
\alert{Vorlesung 15}\\
Gleichheit $\bullet$ Unentscheidbarkeit des Schließens \\[1ex]
\alert{Vorlesung 16}\\
...
\begin{frame}\frametitle{Ausblick: Noch mehr Logik}

\begin{center}
\alert{Vorlesung 20}\\
Korrektheit der Resolution $\bullet$ Grenzen der Prädikatenlogik\\[1ex]
\alert{Vorlesung 21}\\
endliche Modelle $\bullet$ Datenbanken $\bullet$ Komplexität von SQL \\[1ex]
\alert{Vorlesung 22}\\
Datalog $\bullet$ Logik höherer Ordnung \\[1ex]
% 
% Logik höherer Ordnung $\bullet$ entscheidbare Fragmente\\[1ex]
\alert{Vorlesung 23}\\
Modallogiken $\bullet$ entscheidbare Fragmente von Prädikatenlogik\\[1ex]
\alert{Vorlesung 24}\\
% Logik auf natürlichen Zahlen $\bullet$ Gödel
...
\begin{center}
\includegraphics[width=5cm]{images/qrcode-eval-2026.png}
\bigskip

{\large \url{https://tud.link/96s4tp}

}
(Teilnahme bis 21.06.2026 möglich)
\end{center}
...
% \begin{frame}\frametitle{Wo ist der Sinn?}
% 
% \begin{itemize}
...
% \item Wenn wir eine Formel als wahr annehmen, dann beschränken wir damit die möglichen Wertzuweisungen {\tiny da wir nur noch Wertezuweisungen zulassen, welche die Annahmen wahr machen}
...
\begin{itemize}
\item aus "`$\,p_{\textsf{Sokrates-ist-ein-Mensch}}\!$"' wird
\item "`$\textsf{istMensch}(\textsf{sokrates})$"'
\end{itemize}\bigskip\pause

Dadurch können mehrere Dinge dieselbe Eigenschaft haben:
...
\begin{itemize}
\item Was bedeutet $p$?\\
	\visible<2->{Prädikatssymbole stehen für Relationen entsprechender Stelligkeit}
\item Was bedeutet $a$?\\
