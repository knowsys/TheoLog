...
%%%% Notes 2026:
%%%% - Spent extra time on discussing Curry (re functional programming, re "(constrictive) proofs as computations")
%%%% - Spent extra time discussing Z3 example (two-valued entailment decision problems vs. "three-valued" epistemic view vs. two-valued model checking)
...
\item \alert{Terminierung:} Ist sichergestellt, dass die rekursive Berechnung terminiert?\\\pause
	\redalert{Ja}, denn $\textsf{NNF}$ wird in jedem Rekursionsschritt auf Formeln angewendet, die insgesamt weniger Junktoren haben als zuvor.\pause
\item \alert{Korrektheit:} Ist das Ergebnis der rekursiven Umwandlung in NNF?\\\pause
