...
In Italien würde man sagen: "`Se non è vero, è ben trovato`' -- Falls es nicht stimmt, so ist es doch gut erfunden!
...
Die Geschichte zu Coles Vortrag veranschaulicht:\\
\alert{Manche Probleme scheinen schwer zu lösen, aber ihre Lösung ist -- wenn sie erst einmal vorliegt -- leicht zu überprüfen.}
\bigskip

\begin{itemize}
\item Formalisierung durch \alert{polynomielle Verifikatoren}\\$\leadsto$ nachweis-polynomielle Sprachen
\item Satz: eine Sprache ist genau nachweis-polynomiell, wenn sie in $\Scomplclass{NP}$ liegt
\end{itemize}
\bigskip
...
Graph mit bewerteten Kanten einen Pfad durch alle Knoten zu finden, sodass die Summe der Kantenwerte unter einem
...
\alert{Wie stellen wir TM-Berechnungen aussagenlogisch dar?}
\bigskip
...
%
\item Wir definieren einen Graphen $G_F$, sodass gilt:\\[1ex]
