...
\alert{und} des Betrages aller Zahlen in der Enigabe.}
...
\begin{frame}\frametitle{Verwandte ungelöste Probleme}

"`$\Scomplclass{P}\neq\Scomplclass{NP}$?"' ist nur eine von vielen offenen Fragen.
\bigskip\pause

Falls sie zu schwer ist, dann gäbe es zuvor noch ein paar schwächere Resultate zu zeigen:
\begin{itemize}
\item Ist $\Scomplclass{L}\neq\Scomplclass{NP}$?
\item Ist $\Scomplclass{P}\neq\Scomplclass{PSpace}$?
\end{itemize}
\bigskip\pause

Selbst wenn $\Scomplclass{P}\neq\Scomplclass{NP}$ einmal bewiesen wird, dann bleiben immer
noch andere Fragen:
\begin{itemize}
\item Ist $\Scomplclass{NP}\cap\Scomplclass{coNP}\neq\Scomplclass{P}$?
\item Ist $\Scomplclass{NP}\cap\Scomplclass{coNP}\neq\Scomplclass{P}$?
\end{itemize}
