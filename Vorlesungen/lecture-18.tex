...
\defineTitle{18}{Unifikation}{29. Juni 2026}
...
$\lambda$ gibt, sodass $\sigma\circ\lambda =\theta$.\medskip

Der \redalert{allgemeinste Unifikator} für ein Unifikationsproblem $G$ ist ein Unifikator $\sigma$ für $G$, sodass $\sigma\preceq\theta$ für alle Unifikatoren $\theta$ für $G$.}
...
Dagegen ist $\theta=\{x\mapsto f(g(f(b))), y\mapsto g(f(b)),z\mapsto f(b)\}$ ein Unifikator, der nicht allgemeinst ist, denn es gilt zwar $\sigma\{z\mapsto f(b)\}=\theta$,
aber es gibt keine Substitution $\lambda$ mit $\theta\circ\lambda=\sigma$ (denn dazu müsste $\lambda(f(b))=z$ sein, was nicht möglich ist).}
...
\item Fall 2: $s\unieq u$ hat auf keiner Seite eine Variable. Dann hat sie die Form (b), da sonst Zerlegung möglich wäre. (Wir schließen hier den Fall ein, dass eines der Funktionssymbole 0-stellig ist, also eine Konstante.)
\end{itemize}
...
% \emph{Beobachtungen:}
% \begin{itemize}
% \item Eine Menge von Atomen $\mathcal{A}$ ist nur dann unifizierbar, wenn alle Atome das gleiche Prädikat verwenden, d.h. wenn es ein $\ell$-stelliges Prädikatensymbol $p$ gibt, sodass $A_i=p(t_{i,1},\ldots,t_{i,\ell})$ für alle $i\in\{1,\ldots,n\}$.
...
% $\{\neg \textsf{Mensch}(x),\textsf{Mensch}(f(y))\}$ hat den einen Unifikator, sodass wir z.B. die folgende Resolvente erhalten:
